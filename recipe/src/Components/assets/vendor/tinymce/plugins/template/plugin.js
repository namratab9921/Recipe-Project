...
(function () {
  ...
  const getPreviewContent = (editor, html) => {
    ...
    if (html.indexOf('<html>') === -1) {
      ...
      html = '<!DOCTYPE html>' + '<html>' + '<head>' + '<base href="' + encode(editor.documentBaseURI.getURI()) + '">' + contentCssEntries + preventClicksOnLinksScript + '</head>' + '<body className="' + encode(bodyClass) + '"' + dirAttr + '>' + html + '</body>' + '</html>';
    }
